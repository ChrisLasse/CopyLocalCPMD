...
SUBROUTINE Prep_fft_com( comm_send, comm_recv, sendsize, sendsize_rem, comm, nodes_numb, mype, my_node, my_node_rank, node_task_size, buffer_size, send_handle, recv_handle, comm_sendrecv, do_comm, WAVE )
  IMPLICIT NONE
...
s_loop: DO l = 1, howmany_sending( k, i )
     ...
     ELSE
        DO i = 1, buffer_size_save
           DO j = 1, comm_sendrecv(1)
              CALL MPI_REQUEST_FREE( send_handle( j , i , 1 ) )
              CALL MPI_REQUEST_FREE( send_handle( j , i , 2 ) )
           ENDDO
           DO j = 1, comm_sendrecv(2)
              CALL MPI_REQUEST_FREE( recv_handle( j , i , 1 ) )
              CALL MPI_REQUEST_FREE( recv_handle( j , i , 2 ) )
           ENDDO
        ENDDO
        buffer_size_save = buffer_size
     END IF
  ...
  ALLOCATE( send_handle    ( comm_sendrecv(1), buffer_size, 2 ) )
  ALLOCATE( recv_handle    ( comm_sendrecv(2), buffer_size, 2 ) )
  ...
  DO i = 1, buffer_size !INITIALIZE SENDING AND RECEIVING
     ...
     DO j = 1, comm_sendrecv( 1 )
        ...
        CALL mp_send_init_complex( comm_send(:,i), target_node*sendsize, sendsize, comm_info_send( j , mype+1 ) - 1, mype, comm, send_handle( j , i, 1 ) )
     ...
     DO j = 1, comm_sendrecv( 2 )
        ...
        CALL mp_recv_init_complex( comm_recv(:,i), origin_node*sendsize, sendsize, comm_info_recv( j , mype+1 ) - 1, comm, recv_handle( j , i, 1 ) )
  ...
  IF( sendsize_rem .ne. 0 ) THEN
     ...
     DO i = 1, buffer_size !INITIALIZE SENDING AND RECEIVING
        ...
        DO j = 1, comm_sendrecv( 1 )
           ...
           CALL mp_send_init_complex( comm_send(:,i), target_node*sendsize_rem, sendsize_rem, comm_info_send( j , mype+1 ) - 1, mype, comm, send_handle( j , i, 2 ) )
        ...
        DO j = 1, comm_sendrecv( 2 )
           ...
           CALL mp_recv_init_complex( comm_recv(:,i), origin_node*sendsize_rem, sendsize_rem, comm_info_recv( j , mype+1 ) - 1, comm, recv_handle( j , i, 2 ) )
...
SUBROUTINE Make_Manual_Maps( dfft, batch_size, rem_size, nss, my_nr1s, my_nr2s, ngs )
  IMPLICIT NONE

  TYPE(PW_fft_type_descriptor), INTENT(INOUT) :: dfft
  INTEGER, INTENT(IN) :: batch_size, rem_size
  INTEGER, INTENT(IN) :: nss(:), my_nr1s, my_nr2s, ngs
...
! z things
  ...
  DO j = 1, dfft%node_task_size * dfft%nodes_numb
     ...
     DO i = 1+overlap_cor, dfft%nthreads
        dfft%thread_z_sticks( i, 1, j ) = ( nss( j ) * batch_size ) / eff_nthreads
     ENDDO
     ...
     IF( rem_size .ne. 0 ) THEN
        DO i = 1+overlap_cor, dfft%nthreads
           dfft%thread_z_sticks( i, 2, j ) = ( nss( j ) * rem_size ) / eff_nthreads
        ENDDO
...
! y things
  ...
  DO i = 1+overlap_cor, dfft%nthreads
     dfft%thread_y_sticks( i, 1 ) = ( my_nr1s * dfft%my_nr3p * batch_size ) / eff_nthreads
  ENDDO
  DO i = 1+overlap_cor, mod( my_nr1s * dfft%my_nr3p * batch_size, eff_nthreads ) + overlap_cor
  ...
  IF( rem_size .ne. 0 ) THEN
     DO i = 1+overlap_cor, dfft%nthreads
        dfft%thread_y_sticks( i, 2 ) = ( my_nr1s * dfft%my_nr3p * rem_size ) / eff_nthreads
     ENDDO
     DO i = 1+overlap_cor, mod( my_nr1s * dfft%my_nr3p * rem_size, eff_nthreads ) + overlap_cor
...
! x things
  ...
  DO i = 1+overlap_cor, dfft%nthreads
     dfft%thread_x_sticks( i, 1 ) = ( dfft%my_nr3p * my_nr2s * batch_size ) / eff_nthreads
  ENDDO
  DO i = 1+overlap_cor, mod( dfft%my_nr3p * my_nr2s * batch_size, eff_nthreads ) + overlap_cor
  ...
  IF( rem_size .ne. 0 ) THEN
     DO i = 1+overlap_cor, dfft%nthreads
        dfft%thread_x_sticks( i, 2 ) = ( dfft%my_nr3p * my_nr2s * rem_size ) / eff_nthreads
     ENDDO
     DO i = 1+overlap_cor, mod( dfft%my_nr3p * my_nr2s * rem_size, eff_nthreads ) + overlap_cor
