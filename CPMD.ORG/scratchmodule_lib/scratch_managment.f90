...
contains
  ...
  function remove_scratch(id) result(success)
    use scratch_data, only : scratch, scratch_swap
  ...
  subroutine add_remove_association(len,id,start,task,success,arrayin,arrayout)
    use scratch_data, only : scratch,dp
    integer,intent(in) :: len,id,start,task
    logical :: success
    real(dp), pointer, intent(in), optional :: arrayin(:)
    real(dp), pointer, intent(out), optional :: arrayout(:)
  ...
  function move_scratch(src_id,dest_id,src_start,dest_start,len) result(success)
    use scratch_data, only : scratch
    integer,intent(in) :: src_id,dest_id,src_start,dest_start,len
    logical :: success
    integer :: i,deststart,srcstart
    success=.false.
    deststart=dest_start-1
    srcstart=src_start-1
    !get rid of pointer attribute
    success=copy_array(scratch(src_id)%alignednode,scratch(dest_id)%alignednode,srcstart,deststart,len)
  end function move_scratch

  function copy_array(arrayin,arrayout,instart,outstart,len) result(success)
    use scratch_data, only : dp
    integer,intent(in) :: instart,outstart,len
