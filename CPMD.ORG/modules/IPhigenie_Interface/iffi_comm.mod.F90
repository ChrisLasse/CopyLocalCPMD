...
!***************************************************************************
! enable/disable printout of interface progress and other info
#define DBG_PRINT             .false.
! use debug printout for that many steps
#define DBG_IMPEXP            3
...
!     ==--------------------------------------------------------------==
      ...
      IF(DBG_PRINT.AND.paral%parent.AND.runinfo%tverbose) WRITE(6,'(A)') "IFFI_READ"
...
!     of the fixed quantities
        IF(IFIRST.EQ.1) THEN
          IF (DBG_PRINT.AND.paral%parent.AND.runinfo%tverbose) WRITE(6,'(A)') 'INTERFACE| BROADCAST RUNINFO DATA'
...
!         NOTE: IFIRST is set to 1 below 
      ...
      IF (DBG_PRINT.AND.paral%parent.AND.runinfo%tverbose) WRITE(6,'(A)') 'INTERFACE| BROADCAST VAR DATA'
...
!.........broadcast lists only if updated
          IF (epot1_var%tupdateqmintlists) THEN
            IF (DBG_PRINT.AND.paral%parent.AND.runinfo%tverbose) WRITE(6,'(A)') 'INTERFACE| BROADCAST QMMM LISTS'
...
!.........broadcasting epot1_fix
          IF (DBG_PRINT.AND.paral%parent.AND.runinfo%tverbose) WRITE(6,'(A)') 'INTERFACE| BROADCAST FIXED DATA'
...
!.......set default values once
       ...
       IF (INRQMATOMSIFFI.NE.runinfo%nrqmatoms) THEN
         WRITE(6,'(A)') 'WRONG NR OF QMATOMS IN CPMD!'
         WRITE(6,'(A)') 'IFFI:',INRQMATOMSIFFI,'CPMD:',runinfo%nrqmatoms
         WRITE(6,'(A)')'Maybe you have forgotten to include all atoms types'
         WRITE(6,'(A)') 'in you template file.'
...
!.........wfct extrapolation needs to be switched on to have the memory available
          IF (.NOT.cntl%textrap) THEN
            WRITE(6,'(A)') 'runinfo%normalmodeanalysis-MODE NEEDS'
            WRITE(6,'(A)') 'WAVEFUNCTION EXTRAPOLATION TO BE SWITCHED ON.'
...
!......ole parameter
        ...
        IF (VERBOSE.GT.0.0D0) THEN
          ...
          WRITE(6,'(A)') 'INTERFACE| VERBOSE OUTPUT ON!'
        ENDIF
...
!.....boxtrans
      IF (DBG_PRINT.AND.runinfo%tverbose)                                                    &
            WRITE(6,'(A)') 'INTERFACE| SETTING BOX TRANSLATION VECTOR ',           &
...
!     decide whether to extrapolate or not if runinfo%pff is done according to PFFSTATUS 
      ...
      IF (IWRITERESTARTFILE.EQ.1) THEN
          ...
          IF (runinfo%tverbose) WRITE(6,'(A)')  'INTERFACE| writing RESTART file'
      ENDIF
...
!.....determine what to calculate in this run
      IF (runinfo%pff) THEN
       ...
       IF(epot1_var%tnopotandwfctupd) THEN
         WRITE(6,'(A)') 'INTERFACE| NO WAVEFUNCTION UPDATE'
         call STOPGM("READ_RUNINFO","DISCONTINUED",0,"")
!       ELSE
!        WRITE(6,'(A)') 'INTERFACE| WAVEFUNCTION UPDATE'
       ENDIF
      ...
      ENDIF
      IF (epot1_var%calcespcharges.EQ.1.AND.runinfo%tverbose) WRITE(6,'(A)') 'INTERFACE| CALC ESP CHARGES'
      ...
      IF (runinfo%meanfieldmode) THEN
        ...
        ENDIF
      ENDIF
      ! write out summary of steering variables
      IF (runinfo%tverbose) THEN
            IF (IFIRST.EQ.1) THEN
                WRITE(6,'(A)') 'INTERFACE| MODE FR: first run after integration'
                WRITE(6,'(A)') 'INTERFACE| MODE LI: light import (false implies full import) '
                WRITE(6,'(A)') 'INTERFACE| MODE LE: light export (false implies full export) '
                WRITE(6,'(A)') 'INTERFACE| MODE GP: update grid partitioning'
                WRITE(6,'(A)') 'INTERFACE| MODE QI: update qmintlists'
                WRITE(6,'(A)') 'INTERFACE| MODE RG: update radius of gyration'
            ENDIF
            WRITE(6,'(A)') 'INTERFACE|  FR LI LE GP QI RG'
            WRITE(6,'(A,6(L3))') 'INTERFACE|',epot1_var%tdoextrap,&
                                              epot1_var%tcalconlypotfrompff,&
                                              epot1_var%tcalconlypfffield,&
                                              epot1_var%tgridpart,&
                                              epot1_var%tupdateqmintlists.AND..NOT.epot1_var%tcalconlypotfrompff,&
                                              epot1_var%tupdatergyr
...
!.......security check
        IF (epot1_list%nincl(I).GE.maxnear) THEN
           WRITE(6,'(A)') 'INTERFACE| Error: maxnear not big enough!'
           WRITE(6,'(A)') '           Increase this value in iffi.inc'
...
!                  over N=1,epot1_list%nincl(LI) and get  IT = epot1_list%lincl(LI,N) and e.g. epot1_fix%neardata(PCHARGE,IT)
        ...
        ! debug output
        IF (DBG_PRINT.AND.runinfo%tverbose.AND.IFIRSTDBG.GT.0.OR..NOT.defPRINTONLYATSTARTUP) THEN
...
!     counter for printing import/export only at startup
      ...
      if (DBG_PRINT.AND.paral%parent.AND.runinfo%tverbose) WRITE(6,'(A)') "IFFI_WRITE"
...
!........TOIFFI:
      ...
      ENDIF
!.....SCF cycles
      call set_qm_scf_iterations(infi)
!.....lattice vector
...
!.....TOIFFI:
      ...
      IF (DBG_PRINT.AND.runinfo%tverbose.AND.IFIRSTDBG.GT.0) THEN
...
!.....print some information
      IF (paral%parent) THEN
        write(6,'(A,3(I5))')    '  VOXINFO| NUM GRIDPOINTS  :',spar%nr1s,spar%nr2s,spar%nr3s
        write(6,'(A,3(F5.1))') '  VOXINFO| GRIDPOINTS/VOXEL:',DBLE(spar%nr1s)/DBLE(NVOX(1)), &
                     DBLE(spar%nr2s)/DBLE(NVOX(2)),DBLE(spar%nr3s)/DBLE(NVOX(3))

        WRITE(6,'(A,3(I5),4(F8.4))')  '  VOXINFO|NVOX, LVOX (ANGS), RGYR',&
...
!     loop over my part of the grid and assign gridpoints to nearest voxel
      IF (DBG_PRINT.AND.paral%parent)  WRITE(6,'(A)') "ASSIGNING VOXELS TO GRIDPOINTS..."
...
!      calc oleradius from 2*rgyr / eps
      ...
      IF (DBG_PRINT.AND.paral%parent) THEN
