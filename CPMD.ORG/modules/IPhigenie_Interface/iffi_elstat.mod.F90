...
!     ==--------------------------------------------------------------==
      ...
      IF(DBG_PRINT.AND.paral%parent) WRITE (6,'(A)') 'INTERFACE| CALCULATING EXTERNAL POTENTIAL...'
...
!$omp end parallel 
      ...
      IF(DBG_PRINT.AND.paral%parent) WRITE (6,'(A)') 'INTERFACE| ...EXTERNAL POTENTIAL CALCULATED'
...
!==--------------------------------------------------------------==
      IMPLICIT NONE
      CHARACTER(*), PARAMETER                    :: procedureF = 'IF_EXPORT_FULL'
      CHARACTER(*), PARAMETER                    :: procedureL = 'IF_EXPORT_LIGHT'
!     Variables      
      REAL*8  KO_X,KO_Y,KO_Z,DI,DJ,DK
...
!!
      IF (DBG_PRINT.AND.paral%io_parent) WRITE (6,'(3A)') 'INTERFACE| CALCULATING ELECTROSTATICS AT MM-ATOMS...'
...
!     (re)calculate RHOE
      ...
      if (.NOT.runinfo%pff.OR.(.NOT.epot1_var%tcalconlypfffield)) then
        CALC_MM = .TRUE.
        CALL TISET(procedureF,ISUB)
      else
        CALC_MM = .FALSE.
        CALL TISET(procedureL,ISUB)
      endif   

!     zero potential expansion ad MM atoms
      CALL zeroing(MMPOTEXP)!,dimpotexp*maxnear*nthreads)  
      CALL zeroing(MMPOTEXP_VAR)!,DIMPOTEXP_VAR*maxnear*nthreads)  
...
!     zero QM multipole moments
      CALL zeroing(VOMULTEXP)!,dimmultexp*MYNVOX*nthreads)
      CALL zeroing(ATMULTEXP)!,dimmultexp*maxind*nthreads)
...
!     zero QM rgyr moments
      IF (epot1_var%tupdatergyr) THEN
        CALL zeroing(VOXRGYREXP)!,dimrgyrexp*MYNVOX*nthreads)
        CALL zeroing(ATRGYREXP)!, dimrgyrexp*maxind*nthreads)
...
!..................................................
      ...
      IF (DBG_PRINT.AND.paral%io_parent)  WRITE (6,'(3A)') 'INTERFACE| ...ELECTROSTATICS AT MM-ATOMS CALCULATED' 
      if(CALC_MM) THEN
         CALL TIHALT(procedureF,ISUB)
      else
         CALL TIHALT(procedureL,ISUB)
      end if
!==--------------------------------------------------------------==
 RETURN
 END SUBROUTINE
