...
SUBROUTINE vdw_forces(alat, avec, bvec, celvol, natcel, idx_ityp, coorat, &
                      evdw, forces, stress, mepos, nproc, allgrp, error)
  !---------------------------------------------------------------------
  !
...
!#endif
  ...
  INTEGER, INTENT(IN) :: mepos, nproc, allgrp
  ...
  DOUBLE PRECISION :: frc_c6(3), str_c6(3,3), frc_at(3), dE_dCN_at, &
                      dcor(3), ds(3), tt(3), ttn, ttn2, dist1, dist2, &
                      dist4, dist6, dist8, c6, c8, dc6i, dc6j, arg, &
                      tmp, betaRS, expval, damp, efac6, efac8, efac, &
                      cfac, sfac, ffac6, ffac8, ffac, scal
...
!        IF ( (ttn2 > rcut2_vdw) .OR. (abs(ttn2) < 1.d-8) ) CYCLE
        ...
        IF (version(1:2) == 'D2') THEN
          ...
          !
          evdw = evdw - efac
          IF (iat .ne. jat) frc_at(1:3) = frc_at(1:3) + ffac * tt(1:3)
          DO k = 1, 3
          DO i = 1, k
        ...
        ELSE IF (version(1:2) == 'D3') THEN
          ...
          IF (version(7:8) == 'BJ') THEN
            ...
            sfac = ffac * dist1
            !
            dE_dCN_at = dE_dCN_at + efac * (dc6j / c6)
...
!            dE_dC6_sum(iat,jat) = dE_dC6_sum(iat,jat) + efac / c6
    ...
    ENDDO
    !
    forces(1:3,jat) = frc_at(1:3)
    IF (version(1:2) == 'D3') dE_dCN_sum(jat) = dE_dCN_at
    !
  ENDDO
  !
...
!        DO it = 1, nrt
          ...
          sfac = ffac * dist1
          !
          IF (iat .ne. jat) frc_c6(1:3) = frc_c6(1:3) + ffac * tt(1:3)
          DO k = 1, 3
          DO i = 1, k
      ...
      ENDDO
      !
      forces(1:3,jat) = forces(1:3,jat) + frc_c6(1:3)
      !
    ENDDO
    !
