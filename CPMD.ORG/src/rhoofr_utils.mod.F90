...
MODULE rhoofr_utils
  ...
  USE fft,                             ONLY: inzf,&
                                             ...
                                             lr2s,&
                                             lr3s,&
                                             nzff,fft_total,msrays,xf,yf,fft_blocksize,blocking_fft,fft_residual,fft_numblocks,wfn_r,wfn_g
...
! ==--------------------------------------------------------------==
  ...
  ! ==================================================================
  SUBROUTINE rhoofr_blocking(c0,rhoe,psi,nstate)
    ! ==--------------------------------------------------------------==
    ! ==                        COMPUTES                              ==
    ! ==  THE NORMALIZED ELECTRON DENSITY RHOE IN REAL SPACE          ==
    ! ==  THE KINETIC ENERGY EKIN. IT IS DONE IN RECIPROCAL SPACE     ==
    ...
    CHARACTER(*), PARAMETER                  :: procedureN = 'rhoofr_blocking'
    COMPLEX(real_8), PARAMETER               :: zone = (1.0_real_8,0.0_real_8)
    ...
    INTEGER                                  ::  i,ia, iat, ierr, ir, is, is1, is2, ispin(2,fft_blocksize), isub, &
                                                 isub3, iwf, ist, il_wfng(2), il_wfnr(2), il_wfnr1(4), iblock, &
                                                 ir1, bsize, first_state, offset
    REAL(real_8)                             :: chksum, coef3(fft_blocksize), coef4(fft_blocksize), ral, &
                                                rbe, rsp, rsum, rsum1, &
                                                rsum1abs, rsumv, rto, temp(4)
    ...
    CALL request_scratch(il_wfng,wfn_g,procedureN//'wfn_g')
#else
...
#ifdef _USE_SCRATCHLIBRARY
    CALL free_scratch(il_wfng,wfn_g,procedureN//'wfn_g')
#else
    ...
    DO iblock = 1,fft_numblocks +1
       ...
       ! Compute the charge density from the wave functions
       ! in real space
       ! Decode fft blocks, setup (lsd) spin settings
       ispin=1
       offset=first_state-1+(iblock-1)*fft_blocksize*2
       DO ist=1,bsize
          offset=offset+1
          is1=offset
          offset=offset+1
             ...
             DO ist=1,bsize
                DO ir=1,fpar%kr1*fpar%kr2s
                   rhoe_p(ir,ir1,ispin(1,ist))=rhoe_p(ir,ir1,ispin(1,ist))&
                        +coef3(ist)*REAL(wfn_r1(ir,ist,ir1,iblock),KIND=real_8)**2
                   rhoe_p(ir,ir1,ispin(2,ist))=rhoe_p(ir,ir1,ispin(2,ist))&
                        +coef4(ist)*AIMAG(wfn_r1(ir,ist,ir1,iblock))**2
                ENDDO
             END DO
          END DO
       ...
       ELSE
          ...
          DO ir1=1,fpar%kr3s
             DO ist=1,bsize
                DO ir=1,fpar%kr1*fpar%kr2s
                   rhoe_p(ir,ir1,ispin(1,ist))=rhoe_p(ir,ir1,ispin(1,ist))&
                        +coef3(ist)*REAL(wfn_r1(ir,ist+(ir1-1)*bsize,iblock,1),KIND=real_8)**2
                   rhoe_p(ir,ir1,ispin(2,ist))=rhoe_p(ir,ir1,ispin(2,ist))&
                        +coef4(ist)*AIMAG(wfn_r1(ir,ist+(ir1-1)*bsize,iblock,1))**2
                ENDDO
             END DO
          END DO
          IF (lspin2%tlse) THEN
                   ...
                   DO ist=1,bsize
                      ...
                      DO ir=1,fpar%kr1*fpar%kr2s
                         rhoe_p(ir,ir1,ispin(1,ist))=&
                              rhoe_p(ir,ir1,ispin(1,ist))&
                              +coef3(ist)&
                              *REAL(wfn_r1(ir,ist+(ir1-1)*bsize,iblock,1),KIND=real_8)**2
                         rhoe_p(ir,ir1,ispin(1,ist))=&
                              rhoe_p(ir,ir1,ispin(1,ist))&
                              +coef4(ist)*AIMAG(wfn_r1(ir,ist+(ir1-1)*bsize,iblock,1))**2
                      ENDDO
...
#ifdef _USE_SCRATCHLIBRARY
    IF(rsactive)THEN
       call save_scratch(il_wfnr,wfn_r,'wfn_r')
    ELSE
    ...
    ! ==--------------------------------------------------------------==
    RETURN
  END SUBROUTINE rhoofr_blocking
