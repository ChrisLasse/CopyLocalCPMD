...
MODULE vpsi_utils
  ...
  USE fft,                             ONLY: inzf,&
                                             ...
                                             wfn_r,&
                                             wfn_g,&
                                             nzff,fft_total,msrays,xf,yf,fft_blocksize,fft_numblocks,fft_residual
...
CONTAINS
  ...
  SUBROUTINE vpsi(c0,c2,f,vpot,psi,nstate,ikind,ispin,redist_c2)
    ...
    IF (redist_c2) THEN
       ...
       CALL cp_grp_redist_array(C2_vpsi,nkpt%ngwk,nstate)
...
! ==--------------------------------------------------------------==
  ...
  ! ==================================================================
  SUBROUTINE vpsi_blocking(c0,c2,f,vpot,psi,nstate,ikind,ispin,redist_c2)
    ! ==================================================================
    ! == K-POINT AND NOT K-POINT VERSION OF VPSI.                     ==
    ! ==--------------------------------------------------------------==
    ! == VPOT:   IN INPUT POTENTIAL                                   ==
    ...
    CHARACTER(*), PARAMETER                  :: procedureN = 'vpsi_blocking'
    COMPLEX(real_8), PARAMETER               :: zone = (1.0_real_8,0.0_real_8)

    COMPLEX(real_8)                          :: fm, fp, psii, psin
    INTEGER :: i, iclpot = 0, id, ierr, ig, &
      ir, is1, is2, isub, isub2, isub3, iwf, ixx, ixxs, iyy, izz, jj, &
      leadx, lspin(2,fft_blocksize), njump, nnrx, nostat, nrxyz1s, nrxyz2, stream_idx, &
      ist,states_fft,il_wfng(2),il_wfnr1(4), bsize, iblock, istate,ir1,first_state,il_wfnr(2), offset
    REAL(real_8)                             :: chksum, csmult, fi, fip1, g2, &
                                                xskin
    REAL(real_8), ALLOCATABLE                :: vpotx3a(:,:,:), vpotx3b(:,:,:)
    ...
    il_wfng(2)=fft_total
    il_wfnr(1)=fpar%kr1*fpar%kr2s*fpar%kr3s*fft_blocksize
    il_wfnr(2)=fft_numblocks
    IF (fft_residual .gt. 0) il_wfnr(2)=fft_numblocks+1
#ifdef _USE_SCRATCHLIBRARY
    IF(rsactive)THEN
       CALL request_saved_scratch(il_wfnr,wfn_r,'wfn_r')
    ELSE
       CALL request_scratch(il_wfnr,wfn_r,'wfn_r')
    END IF
    CALL request_scratch(il_wfng,wfn_g,procedureN//'_wfn_g')
#else
    ...
    IF(.NOT.rsactive) THEN
       ...
       ! ==  time.                                                       ==
       ! ==--------------------------------------------------------------==
       CALL invfftn_block(wfn_g,wfn_r)
    END IF

    il_wfnr1(1)=fpar%kr1*fpar%kr2s
    il_wfnr1(2)=fft_blocksize
    il_wfnr1(3)=fpar%kr3s
    il_wfnr1(4)=fft_numblocks
    IF (fft_residual .gt. 0) il_wfnr1(4)=fft_numblocks+1
    ...
    DO iblock = 1,fft_numblocks +1
       IF ( fft_residual .EQ. 0 .AND. iblock .GT. fft_numblocks ) CYCLE
       bsize=fft_blocksize
       IF ( iblock .GT. fft_numblocks) bsize=fft_residual
       ! ==------------------------------------------------------------==
       ! == Apply the potential (V), which acts in real space.         ==
       IF ( iblock .LE. fft_numblocks) THEN
          lspin=1
          offset=first_state-1+(iblock-1)*fft_blocksize*njump
          DO ist=1,bsize
             offset=offset+1
             is1=offset
             IF (njump.EQ.2) THEN 
          ...
          DO ir1=1,fpar%kr3s
             DO ist=1,bsize
                DO ir=1,fpar%kr1*fpar%kr2s
                   wfn_r1(ir,ist,ir1,iblock)=&
                        vpotdg(ir,ir1,lspin(1,ist))*REAL(wfn_r1(ir,ist,ir1,iblock),KIND=real_8)&
                        +uimag*vpotdg(ir,ir1,lspin(2,ist))*AIMAG(wfn_r1(ir,ist,ir1,iblock))
                ENDDO
             END DO
          END DO
          IF (td_prop%td_extpot.AND.cntl%tlsd.AND.ispin.EQ.2) THEN
             ...
             IF (ist .LE. bsize) THEN
                ...
                DO ir1=1,fpar%kr3s
                   DO ir=1,fpar%kr1*fpar%kr2s
                      wfn_r1(ir,ist,ir1,iblock)=&
                           extf_p(ir,ir1)*REAL(wfn_r1(ir,ist,ir1,iblock),KIND=real_8)&
                           +uimag*extf_p(ir,ir1)*AIMAG(wfn_r1(ir,ist,ir1,iblock))
                   END DO
                END DO
             END IF
          END IF
       ELSE
          ...
          !=> iblock = fft_numblocks+1
          il_wfnr1(1)=fpar%kr1*fpar%kr2s
          il_wfnr1(2)=fft_blocksize*fpar%kr3s
          il_wfnr1(3)=fft_numblocks+1
          il_wfnr1(4)=1
          CALL reshape_inplace(wfn_r, (/il_wfnr1(1),il_wfnr1(2),il_wfnr1(3),il_wfnr1(4)/), wfn_r1)
          lspin=1
          offset=first_state-1+(iblock-1)*fft_blocksize*njump
          ...
          DO ir1=1,fpar%kr3s
             DO ist=1,bsize
                DO ir=1,fpar%kr1*fpar%kr2s
                   wfn_r1(ir,ist+(ir1-1)*bsize,iblock,1)=&
                        vpotdg(ir,ir1,lspin(1,ist))*REAL(wfn_r1(ir,ist+(ir1-1)*bsize,iblock,1),KIND=real_8)&
                        +uimag*vpotdg(ir,ir1,lspin(2,ist))*AIMAG(wfn_r1(ir,ist+(ir1-1)*bsize,iblock,1))
                END DO
             END DO
          END DO
          IF (td_prop%td_extpot.AND.cntl%tlsd.AND.ispin.EQ.2) THEN
             ...
             IF (ist .LE. bsize) THEN
                ...
                DO ir1=1,fpar%kr3s
                   DO ir=1,fpar%kr1*fpar%kr2s
                      wfn_r1(ir,ist+(ir1-1)*bsize,iblock,1)=&
                           extf_p(ir,ir1)*REAL(wfn_r1(ir,ist+(ir1-1)*bsize,iblock,1),KIND=real_8)&
                           +uimag*extf_p(ir,ir1)*AIMAG(wfn_r1(ir,ist+(ir1-1)*bsize,iblock,1))
                   END DO
                END DO
             END IF
          END IF
    ...
    call free_scratch(il_wfng,wfn_g,procedureN//'_wfn_g')
#else
